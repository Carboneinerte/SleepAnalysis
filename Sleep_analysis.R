...
Sleep_analysis = function(file_path,graphs_){
...
onset_W_list = c()
for (t in date){
  Sub_onset = subset(RawData, date == t & ZT == 12)
  if (nrow(Sub_onset != 0)){
    Sub_onset$rodent_sleep[Sub_onset$rodent_sleep == "P"] = "S"
    Sub_onset[,1:2] = NULL
    Sub_onset[,3:17] = NULL
    ...
    wake_episode = c()
    for (x in 1:400){
      if (Sub_onset$rodent_sleep[x] == "W" & Sub_onset$rodent_sleep[x+1] == "W" &
          Sub_onset$rodent_sleep[x+2] == "W" ){
        wake_episode[x]=1
...
Sleep_select = function(criteria = "csv",graphs_="no"){
