...
bouts_df = RawData
bouts_df$durations = 0  #new column to store the durations
current_letter <- ""
current_duration <- 0

# Loop through each row and calculate the durations
for (i in 1:nrow(bouts_df)) {
...
# Extract the durations each time the letter changes
...
bouts_df_co = data.frame(bouts_df$date,bouts_df$ZT,bouts_df$rodent_sleep,bouts_df$max_dur)
bouts_df_co <- bouts_df_co[complete.cases(bouts_df_co), ]
names(bouts_df_co) = c("Date","ZT","Sleep state","Bouts Duration")
bouts_df_co$'Duration (min)' = bouts_df_co$`Bouts Duration`*0.16666666666666666666666666666666666666
...
for (y in ZT){
  ...
  names(Temperature_mean_table) = c("Temperature_Mean","Temperature_SD","Temperature_N")
}
...
for (y in ZT){
  ...
  names(EMG_mean_table) = c("EMG_Mean","EMG_SD","EMG_N")
}
...
for (y in ZT){
  ...
  names(Alpha_mean_table) = c("Alpha_Mean","Alpha_SD","Alpha_N")
}
...
for (y in ZT){
  ...
  names(Beta_mean_table) = c("Beta_Mean","Beta_SD","Beta_N")
}
...
for (y in ZT){
  ...
  names(Delta_mean_table) = c("Delta_Mean","Delta_SD","Delta_N")
}
...
for (y in ZT){
  ...
  names(Sigma_mean_table) = c("Sigma_Mean","Sigma_SD","Sigma_N")
}
...
for (y in ZT){
  ...
  names(Theta_mean_table) = c("Theta_Mean","Theta_SD","Theta_N")
}
...
addWorksheet(wb2, "Temperature - ZT - Mean")
writeData(wb2, "Temperature - ZT - Mean", x=Temperature_ZT_Mean)

if (dir.exists("output")==FALSE){
  dir.create("output")
}
ID = gsub("/","_", file_path)
filename = paste0("output/",ID,".xlsx")
saveWorkbook(wb2 , file = filename, overwrite = T)

print("Successful Analysis of")
print(ID)
}
